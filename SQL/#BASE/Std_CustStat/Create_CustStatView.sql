...
SELECT ITable.ITEMID	   + ' ' + ITable.ITEMNAME AS Item, 
       ...
       CASE WHEN sum(CIT.QTY) <> 0 THEN	sum(CIT.sumlinediscmst) * (-1*SUM(ITrans.QTY)/sum(CIT.QTY))
		...
		CTable.COUNTRY as Country,
		CIJ.LEDGERVOUCHER as LedgerVoucher,
		--ITrans.recid,
		1 AS CNT
                      
FROM CUSTINVOICETRANS CIT INNER JOIN
	CUSTINVOICEJOUR CIJ ON CIJ.INVOICEDATE = CIT.INVOICEDATE AND CIJ.INVOICEID = CIT.INVOICEID AND CIJ.DATAAREAID = CIT.DATAAREAID INNER JOIN
	...
	INVENTTRANS ITrans ON ITrans.INVENTTRANSID = CIT.INVENTTRANSID AND ITrans.INVOICEID = CIJ.INVOICEID AND ITrans.DATEFINANCIAL = CIJ.INVOICEDATE AND ITrans.DATAAREAID = CIT.DATAAREAID INNER JOIN
...
GROUP BY ITable.ITEMID, ITable.ITEMNAME, ITable.ITEMGROUPID, IGrp.NAME, ITrans.CUSTVENDAC, CTable.NAME, CTable.SALESGROUP,
	...
	CIJ.LEDGERVOUCHER, CTable.COUNTRY