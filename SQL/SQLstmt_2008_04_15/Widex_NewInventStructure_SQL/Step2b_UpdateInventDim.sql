...
SET NOCOUNT ON
Exec dbo.LogProgressInfo ' Step 2b - Nye inventdim startet','Step2b',1
begin tran
exec droptable 'InventDimTmp'
go
select count(*) as cnt, INVENTBATCHID, WMSLOCATIONID, WMSPALLETID, INVENTLOCATIONID, DATAAREAID, 
						999999999 AS RECID, '01234567890123456789' AS INVENTDIMID -- MOG: 08/04-2008 20 tegn
	into InventDimTmp
	FROM         INVENTDIM
...
update dbo.InventDimTmp set @NewRecid = @NewRecid + 1,
				 recid			  = @NewRecid,
			     @DimId		      = CAST(@NextInventDimId as VARCHAR(20)),
			     InventDimID      = dbo.RLen(@DimId,20), -- MOG: 08/04-2008
...
commit tran
Exec dbo.LogProgressInfo ' Step 2b - Nye inventdim afsluttet','Step2b',2
GO
