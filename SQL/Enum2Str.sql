...
BEGIN
 ...
 SET @bin = (SELECT TOP 1 Properties
    FROM [UAT2_AX2012_model].dbo.ModelElement me
    JOIN [UAT2_AX2012_model].dbo.ModelElementData med
 ...
 IF SUBSTRING(@ret, 1, 1) = '@' --label file
	BEGIN
		...
		SET @ret = (SELECT TOP 1 Text FROM [UAT2_AX2012_model].dbo.ModelElementLabel
					WHERE LabelId = @label
						AND Module = @module
						AND Language = 'en_us'
